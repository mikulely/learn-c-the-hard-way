...
// description : command part
// created at: Sat 14 Dec 2013 08:39:38 PM CST
...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only){
        ...
        if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS){
                ...
                check(!fetch_only, "No point in fecthing a DEPENDS file.");
                ...
                apr_status_t rc = apr_dir_make_recursive(BUILD_DIR,
                                                         APR_UREAD|APR_UWRITE|APR_UEXECUTE, p);
                check(rc == 0, "Failed to make dir %s", BUILD_DIR);
...
int Command_build(apr_pool_t *p, const char *url, const char *configure_opts,
        ...
        check(access(BUILD_DIR, X_OK|R_OK|W_OK) == 0,
              "Build dir doesn't exists: %s", BUILD_DIR);
